...
from looptrace_regionals_vis.reader import parse_boxes

BOX_CENTER_COLNAMES = ["zc", "yc", "xc"]
...
@pytest.mark.parametrize(
    ...
    [
        cols
        for k in range(1, len(BOX_CENTER_COLNAMES) + 1)
        for cols in itertools.combinations(BOX_CENTER_COLNAMES, k)
    ],
)
...
def test_parse_is_robust_to_presence_or_absence_of_index_column(tmp_path, spots_file):
    boxes_1: list[BoundingBox3D] = parse_boxes(spots_file)
    target = tmp_path / spots_file.name
    pd.read_csv(spots_file, index_col=0).to_csv(target, index=False)
    boxes_2: list[BoundingBox3D] = parse_boxes(target)
